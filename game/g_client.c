...
char *ClientConnect( int clientNum, qboolean firstTime, qboolean isBot ) {
	...
	// ## BOT CODE ##
	// kaji : Bots dont have or need cl_guid. This check would put "" into the array
	// resulting into a ban for the next bot that joins the game.
	// So just gently ignore this check for bots.
...
void ClientBegin( int clientNum ) {
	...
	// ### BOT CODE ###
	// kaji : modification to GAME CODE , bots have a pain function so we can easily tell
	// if an enemy is shooting at us
	if( ent->botClient )
	{
		ent->pain = Bot_TakeDamage;

		if( g_debugBots.integer )
		{
			G_Printf("Setting PAIN Pointer\n");
		}
	}
	else
	{
	...
	// ## BOT CODE ##
	// kaji : see my comment above in ClientConnect
	if( !(g_entities[clientNum].r.svFlags & SVF_BOT) )
	{
